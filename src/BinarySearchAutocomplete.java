...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PrefixComparator comps = PrefixComparator.getComparator(prefix.length());
		...
		for (int index = first; index<= last; index++) {
			if (!myTerms[index].getWord().startsWith(prefix))
				continue;
			if (pq.size() < k) {pq.add(myTerms[index]); }
			else if (pq.peek().getWeight() < myTerms[index].getWeight()) {
				pq.remove();
				pq.add(myTerms[index]);
			}
		}
